...
/*   Updated: 2024/06/27 22:49:38 by djanusz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int ac, char **av)
{
	...
	/* ft_write */
	tmp = write(1, av[1], strlen(av[1]));
	printf("\nwrite(1, \"%s\", strlen(av[1])) = %d (errno = %d)\n", av[1], tmp, errno);
	tmp = ft_write(1, av[1], ft_strlen(av[1]));
	printf("\nft_write(1, \"%s\", ft_strlen(av[1])) = %d (errno = %d)\n\n", av[1], tmp, errno);

	/* ft_read */
	tmp = read(-1, g_buffer, BUFFER_SIZE);
	printf(" | read(-1, \"%s\", BUFFER_SIZE) = %d (errno = %d)\n", g_buffer, tmp, errno);
	buffer_reset();
	tmp = ft_read(-1, g_buffer, BUFFER_SIZE);
